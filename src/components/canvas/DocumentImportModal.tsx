...
function estimateImport(text: string) {
  const { nodes, edges } = parseDocumentToCanvas(text);
  return {
    nodes: nodes.length,
    edges: edges.length,
  };
...
export function DocumentImportModal({ isOpen, onClose }: DocumentImportModalProps) {
  ...
  const handleImport = async () => {
    ...
    setIsImporting(true);
    try {
      const { nodes, edges } = parseDocumentToCanvas(trimmed);
      if (nodes.length === 0) {
        ...
        <div className="px-6 pb-6 pt-4">
          ...
          {preview && (
            <div className="mt-3 text-xs text-slate-400">
              Preview: {preview.nodes} nodes, {preview.edges} edges
            </div>
          )}
